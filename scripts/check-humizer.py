...
# Транскрипція з опитування має мітки реплік «Прізвище: текст» (перевірено: 159 із 327
# записів, «Speaker N» — один). Fathom і tldv лишаються: старі транскрипти нікуди не зникли.
TRANSCRIPT = re.compile(r'Speaker\s*\d|fathom\.video|fathom|tldv|tl;dv|\b\d{1,2}:\d{2}:\d{2}\b', re.I)
# мітка репліки: «Ім'я Прізвище:» — два слова з великої, потім двокрапка.
# ОДНА така мітка нічого не доводить: у клієнтському документі законно стоїть
# «Нова Пошта: обмін документами» — на цьому перевірка вже давала фальшиве падіння
# на реальній фікстурі. Ознака діалогу — мітка, що повторюється, або дві різні.
SPEAKER_UA = re.compile(r'[А-ЯІЇЄҐ][а-яіїєґʼ\']+\s+[А-ЯІЇЄҐ][а-яіїєґʼ\']+\s*:\s')
...
def check(path):
    ...
    labels = [x.strip() for x in SPEAKER_UA.findall(txt)]
    uniq = dict.fromkeys(labels)
    if len(uniq) >= 2 or any(labels.count(x) >= 2 for x in uniq):
        bad.append('мітки реплік транскрипції: ' + ', '.join(f'«{x}»' for x in list(uniq)[:3])
                   + ' (§3, §12 конвенцій)')
    m = ASKS.search(txt)
    if m:

...
for f in $(our_skill_files); do
  ...
  for c in 'Обсяг у межах' 'Мова за' 'Повторний прогін' 'Питання до сейла'; do
  ...
  # --- §11 конвенцій: хто спирається на поля картки, той її сам і знаходить ---
  if grep -q 'crm\.lead' "$f"; then
    # §11 має бути саме у блоці читання: у Pre-save воно теж є, і перевірка «десь у файлі»
    # лишалась зеленою після видалення правила зі входу
    read_block=$(awk '/^\*\*Читання вхідних/{inb=1; next} inb && (/^\*\*/ || /^---$/){exit} inb' "$f")
    grep -qE 'Картка ліда.*§11' <<<"$read_block" || { echo "  ✗ у блоці «Читання вхідних…» немає правила «Картка ліда … §11 конвенцій»"; fail=1; }
    grep -qE '^- \[ \] \*\*Картка ліда' "$f" || { echo "  ✗ читає картку, але в Pre-save немає рядка «Картка ліда … §11 конвенцій»"; fail=1; }
    if grep -qE '\["duplicate_lead_count"' "$f"; then
      echo "  ✗ фільтрує по duplicate_lead_count — поле обчислюване, домен збігається з усіма картками (§11 конвенцій)"; fail=1
    fi
  fi

  # --- §12 конвенцій: хто працює з транскриптом, той знає три шари джерела ---
  # read_block уже витягнутий вище, якщо файл згадує crm.lead; тут витягуємо власний
  rb=$(awk '/^\*\*Читання вхідних/{inb=1; next} inb && (/^\*\*/ || /^---$/){exit} inb' "$f")
  if grep -qiE 'транскрипт|fathom' <<<"$rb"; then
    grep -qE '§12' <<<"$rb" || { echo "  ✗ у блоці «Читання вхідних…» є транскрипт або Fathom, але немає посилання на §12 конвенцій (транскрипція вже в базі)"; fail=1; }
    # §12 без моделі — це посилання без дії: транскрипція лежить у survey.user_input
    grep -q 'survey\.user_input' <<<"$rb" || { echo "  ✗ працює з транскриптом, але не читає survey.user_input — транскрипція вже в базі (§12 конвенцій)"; fail=1; }
  fi
  # tl;dv більше не інструмент дому: у прозі його не залишаємо
  grep -qE 'tl;dv|tldv' "$f" && { echo "  ✗ згадує tl;dv — інструмент запису дому Fathom (§12 конвенцій)"; fail=1; }

  # --- інваріант 9: хто множить години на ставку, той знає базову 50 €/год ---
  if grep -qE 'сума в євро|× ставка|вилка в євро|межа торгу' "$f"; then
    grep -qE '50 €/год' "$f" || { echo "  ✗ рахує суму в євро, але не знає базової ставки 50 €/год (інваріант 9 контракту журналу)"; fail=1; }
  fi

  # --- §13: обсяг роботи узгоджується ДО роботи ---
  # хто віддає клієнтський .docx — узгоджує склад пакета перед збіркою
  if grep -q '\.docx' <<<"$save_block"; then
    grep -qE '^- \[ \] \*\*Склад (пакета|перевипуску) узгоджений' "$f" || { echo "  ✗ збирає пакет .docx, але в Pre-save немає рядка «Склад … узгоджений до збірки (§13)»"; fail=1; }
  fi
  # писар плану демо-збірки і той, хто з нього збирає, тримають рівень бази
  if grep -q 'план_демо-збірки' <<<"$save_block" || grep -qE 'погоджений .?план_демо-збірки' "$f"; then
    grep -q 'Рівень бази' "$f" || { echo "  ✗ працює з планом демо-збірки, але не тримає рівень бази (§13 конвенцій, канонічне питання 2)"; fail=1; }
  fi

  # --- сценарій демо мусить лишатись покроковим, а не «планом показу» ---
  if grep -q 'план_презентації' <<<"$save_block"; then
    for marker in 'Де в Odoo' 'Кліки:' 'Якщо не працює:' 'Перехід далі:' 'Хронометраж'; do
      grep -qF -- "$marker" "$f" || { echo "  ✗ сценарій демо без «$marker» — за таким документом не проведеш зустріч (Крок 1)"; fail=1; }
    done
  fi
  # знімок — джерело шляхів і кліків для сценарію
  if grep -q 'знімок_демо' <<<"$save_block"; then
    grep -qF -- 'Де в меню' "$f" || { echo "  ✗ знімок не фіксує шлях у меню — сценарію нема звідки брати кліки"; fail=1; }
  fi
